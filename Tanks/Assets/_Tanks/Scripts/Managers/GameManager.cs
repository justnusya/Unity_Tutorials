...
namespace Tanks.Complete
{
    public class GameManager : MonoBehaviour
    {
        ...
        public void ReactiveAllPowerUps()
        {
            // Знаходимо абсолютно всі об'єкти зі скриптом PowerUp на сцені, 
            // навіть ті, що зараз вимкнені (параметр true)
            PowerUp[] allPowerUps = Resources.FindObjectsOfTypeAll<PowerUp>();

            foreach (PowerUp pu in allPowerUps)
            {
                // Перевіряємо, чи це реальний об'єкт у сцені, а не префаб у папці Project
                if (pu.gameObject.scene.name != null)
                {
                    pu.ResetPowerUp();
                }
            }
