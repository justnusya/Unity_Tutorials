...
namespace Tanks.Complete
{
    public class PowerUpDetector : MonoBehaviour
    {
        ...
        private IEnumerator IncreaseSpeed(float speedBoost, float TurnSpeedBoost, float duration)
        {
            ...
            if (m_TankMovement.Rigidbody != null)
            {
                m_TankMovement.Rigidbody.angularVelocity = Vector3.zero;
                // Це зупинить будь-який інерційний рух, спричинений зіткненням з бонусом
                m_TankMovement.Rigidbody.linearVelocity = Vector3.ClampMagnitude(m_TankMovement.Rigidbody.linearVelocity, m_TankMovement.m_Speed);
            }
